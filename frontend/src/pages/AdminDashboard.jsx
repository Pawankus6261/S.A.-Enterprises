...
import { LogOut, Plus, Search, Edit2, Trash2, X, ChevronLeft, ChevronRight, UserCheck, History, Eye, MinusSquare, PlusSquare, Wallet, TrendingUp, Users, Phone, MapPin, UserPlus, PlusCircle, Calendar, Home, Snowflake, FileText, Printer, CheckCircle, AlertCircle, AlertTriangle } from 'lucide-react';
...
export default function AdminDashboard() {
    ...
    const safeRates = rates || { normal: 20, chilled: 30 };
    ...
    // --- BACK BUTTON HANDLER ---
    useEffect(() => {
        if (showInvoice || viewingCustomer || modalType || confirmModal) {
    ...
    }, [showInvoice, viewingCustomer, modalType, confirmModal]);
    ...
    const handleRateChange = (type, change) => {
        const newNormal = type === 'normal' ? safeRates.normal + change : safeRates.normal;
        const newChilled = type === 'chilled' ? safeRates.chilled + change : safeRates.chilled;
    ...
    const handlePaymentToggle = async (status) => {
        ...
        showToast(status ? "Month marked as PAID" : "Month marked as UNPAID", status ? 'success' : 'error');
    };

    const handleRegister = async (e) => {
        e.preventDefault();
        if (await registerConsumer(regForm)) {
            setModalType(null);
            setRegForm({ name: '', mobile: '', house_no: '', area: '', custom_rate: '' });
            showToast('Consumer Added Successfully!');
        }
    };

    const handleUpdateConsumer = async (e) => {
        e.preventDefault();
        if (await updateConsumer(editingId, regForm)) {
            setModalType(null);
            setEditingId(null);
    ...
    const promptDeleteConsumer = () => {
        setConfirmModal({
            title: "Delete Consumer?",
            message: "This will permanently delete this consumer and ALL their history. This action cannot be undone.",
            onConfirm: async () => {
                ...
                showToast("Consumer Deleted", "error");
            }
        });
    };
    ...
    const getEffectiveRate = (consumer, type) => {
        if (type === 'chilled') return safeRates.chilled;
        if (consumer && consumer.custom_rate) return consumer.custom_rate;
        return safeRates.normal;
    };
                ...
                <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[60] flex items-center justify-center p-4 animate-in fade-in">
                    <div className="bg-white w-full max-w-sm rounded-[2rem] shadow-2xl p-6 text-center">
                        ...
                        <div className="flex gap-3">
                            <button onClick={() => setConfirmModal(null)} className="flex-1 py-3 rounded-xl font-bold bg-slate-100 text-slate-600 hover:bg-slate-200 transition">Cancel</button>
                            <button onClick={confirmModal.onConfirm} className="flex-1 py-3 rounded-xl font-bold bg-red-500 text-white hover:bg-red-600 shadow-lg shadow-red-200 transition">Delete</button>
                        </div>
                    </div>
                </div>
            )}
                    ...
                    <div className="w-full xl:w-auto flex flex-col md:flex-row justify-center gap-3">
                        <div className="flex items-center gap-4 bg-slate-900/5 border border-white/50 px-4 py-2 rounded-xl backdrop-blur-md justify-between">
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div className="min-w-[40px] text-center"><span className="text-xl font-black text-slate-800">₹{safeRates.normal}</span></div>
                        ...
                        <div className="flex items-center gap-4 bg-cyan-900/5 border border-cyan-100 px-4 py-2 rounded-xl backdrop-blur-md justify-between">
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div className="min-w-[40px] text-center"><span className="text-xl font-black text-cyan-800">₹{safeRates.chilled}</span></div>
                    ...
                    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-50 flex items-center justify-center p-4 animate-in fade-in print:hidden">
                        <div className="bg-white w-full max-w-md rounded-[2rem] shadow-2xl p-8 relative">
                            ...
                            <form onSubmit={modalType === 'edit_consumer' ? handleUpdateConsumer : handleRegister} className="space-y-4">
                                ...
                                <div><label className="text-xs font-bold text-slate-400 uppercase ml-1">Custom Rate (Normal)</label><input type="number" placeholder={`Default: ₹${safeRates.normal}`} className="w-full p-3 bg-yellow-50 border border-yellow-100 text-yellow-800 rounded-xl font-bold" value={regForm.custom_rate} onChange={e => setRegForm({ ...regForm, custom_rate: e.target.value })} /></div>
                                <div className="flex gap-3 mt-4">
                                    {modalType === 'edit_consumer' && <button type="button" onClick={promptDeleteConsumer} className="flex-1 bg-red-100 text-red-600 py-3 rounded-xl font-bold flex items-center justify-center gap-2 hover:bg-red-200"><Trash2 size={18} /> Delete</button>}
                                ...
                                <form onSubmit={modalType === 'edit_entry' ? handleEditEntrySubmit : handleEntrySubmit} className="space-y-4">
                                    ...
                                    <div className="grid grid-cols-2 gap-3">
                                        <button type="button" onClick={() => setEntryForm({ ...entryForm, type: 'normal' })} className={`p-4 rounded-2xl border-2 flex flex-col items-center gap-1 ${entryForm.type === 'normal' ? 'border-blue-500 bg-blue-50 text-blue-700' : 'border-slate-100 text-slate-400'}`}>
                                            <span className="font-black">Normal</span><span className="text-xs font-bold opacity-70">₹{selectedConsumer?.custom_rate || safeRates.normal}</span>
                                        </button>
                                        <button type="button" onClick={() => setEntryForm({ ...entryForm, type: 'chilled' })} className={`p-4 rounded-2xl border-2 flex flex-col items-center gap-1 ${entryForm.type === 'chilled' ? 'border-cyan-500 bg-cyan-50 text-cyan-700' : 'border-slate-100 text-slate-400'}`}>
                                            <span className="font-black flex items-center gap-1"><Snowflake size={14} /> Chilled</span><span className="text-xs font-bold opacity-70">₹{safeRates.chilled}</span>
                                        </button>
                                    </div>
                                    <div className="grid grid-cols-2 gap-3">
                                    ...
                                    <div className="flex gap-3">
                                        {modalType === 'edit_entry' && <button type="button" onClick={promptDeleteEntry} className="flex-1 bg-red-100 text-red-600 py-3 rounded-xl font-bold flex items-center justify-center gap-2 hover:bg-red-200 transition"><Trash2 size={18} /> Delete</button>}
                                    ...
                                    <div className="bg-slate-900 text-white p-6 md:p-8 relative shrink-0">
                                        ...
                                        <div className="relative z-10 mt-2">
                                            ...
                                            <div className="grid grid-cols-2 gap-3 mt-4">
                                                ...
                                                <button onClick={() => { setShowInvoice(true); }} className="bg-blue-600 text-white font-bold py-3 rounded-xl flex items-center justify-center gap-2 hover:bg-blue-500 transition shadow-lg shadow-blue-900/20"><FileText size={18} /> Generate Bill</button>
                                            </div>
                                        </div>
                                    </div>
                                                    ...
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        <div>
                                                            ...
                                                            <p className="text-[10px] font-bold text-slate-400 uppercase">
                                                                {e.qty} Jars {e.type === 'chilled' && <span className="text-cyan-500">(Chilled)</span>}
                                                                {e.is_paid && <span className="ml-2 text-[8px] bg-green-100 text-green-700 px-1 rounded">PAID</span>}
                                                            </p>
                                                        </div>
                                                    </div>
